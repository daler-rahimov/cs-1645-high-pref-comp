...
pthread_mutex_t *arrRemoveLocks; //locked until corresponding data file in loaded to the memory
pthread_mutex_t *arrSubLocks; //locked until corresponding data file in loaded to the memory
...
void *remove_t_func() {
	...
	for (j = 0; j < NUM_FILES; j++) {
		...
		pthread_mutex_unlock(&arrRemoveLocks[j]);
//		printf("%s\n", allFileData[j]);
	}		// all data
}
...
void *sub_t_func() {
	...
	for (j = 0; j < NUM_FILES; j++) {
		...
		pthread_mutex_unlock(&arrLoadLocks[j]);		// unlock
		pthread_mutex_unlock(&arrSubLocks[j]);
//		printf("%s\n", allFileData[j]);
...
void *count_t_func() {
	...
	int j;
	for (j = 0; j < NUM_FILES; j++) {
		pthread_mutex_lock(&arrRemoveLocks[j]);
		pthread_mutex_lock(&arrSubLocks[j]);
		/* get the first token */
...
int main() {
	words = calloc(MAXWORDS, sizeof(word));
	int i;
...
// allocate memory for mutex arrays and lock all of them
	...
	arrRemoveLocks = calloc(NUM_FILES, sizeof(pthread_mutex_t));
	...
	for (i = 0; i < NUM_FILES; i++) {
		...
		if (pthread_mutex_init(&arrRemoveLocks[i], NULL) != 0) {
			fprintf(stderr, "mutex init failed\n");
			exit(1);
		}
		pthread_mutex_lock(&arrRemoveLocks[i]);
